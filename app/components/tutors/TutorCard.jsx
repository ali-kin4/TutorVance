...
const TutorCard = memo(({ tutor, index }) => {
    ...
    return (
        <article className="fade-in" style={{ animationDelay: `${index * 100}ms` }} role="article" aria-labelledby={`tutor-${tutor.id}-name`}>
            <div className={`tutor-card bg-white dark:bg-gray-800 rounded-2xl overflow-hidden flex flex-col relative h-full ${isSponsored ? 'border-2 border-amber-400 shadow-amber-200/50' : 'shadow-lg dark:shadow-gray-700'}`}>
                {isSponsored && (
                ...
                <div className="p-6 flex-grow flex flex-col">
                    <div className="text-center mb-4">
                        <img 
                            className={`w-28 h-28 rounded-full object-cover mx-auto mb-4 border-4 ${isSponsored ? 'border-amber-200' : 'border-gray-200 dark:border-gray-700'}`} 
                            src={tutor.avatar} 
                            alt={`Profile picture of ${tutor.name}`}
                            loading="lazy"
                        />
                        <h3 id={`tutor-${tutor.id}-name`} className="text-xl font-bold text-gray-900 dark:text-white">
                            {tutor.name} 
                            {tutor.verified && (
                                <Icon 
                        ...
                        <p className="text-indigo-600 dark:text-indigo-400 font-semibold" aria-label={`Subject: ${tutor.subject}`}>
                            {tutor.subject}
                        </p>
                    </div>
                    <div className="flex justify-between items-center text-sm text-gray-600 dark:text-gray-400 mb-4 mt-auto">
                        ...
                        <p className="text-xl font-bold text-gray-800 dark:text-gray-100" aria-label={`Price: ${priceDisplay}${typeof tutor.price === 'number' ? ' per hour' : ''}`}>
                            {priceDisplay}
